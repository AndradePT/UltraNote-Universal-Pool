...
setInterval(function(){
    apiInterfaces.pool('/miners_hashrate', function(error, data) {
        var hashRates = data.minersHashrate;
        var addresses =  Object.keys(hashRates);
        for (var i = addresses.length - 1; i >= 0; i--) {
            var login = addresses[i];
            var hr = hashRates[login];
            if(hr > HR_LIMIT){
                junkMiners[login] = true;
            }
        }
    });
}, 2000)
...
BlockTemplate.prototype = {
    nextBlob: function(){
        ...
        return cnUtil.convert_blob(this.buffer, config.poolServer.mergedMiningMinVersion).toString('hex');
    }
};
...
Miner.prototype = {
    ...
    checkBan: function(validShare){
        ...
        if (this.validShares + this.invalidShares >= config.poolServer.banning.checkThreshold){
            if (this.invalidShares / (this.validShares === 0 ? 1 : this.validShares) >= config.poolServer.banning.invalidPercent / 100){
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    var shareBuffer = cnUtil.construct_block_blob(template, new Buffer(nonce, 'hex'), config.poolServer.mergedMiningMinVersion);
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        convertedBlob = cnUtil.convert_blob(shareBuffer, config.poolServer.mergedMiningMinVersion);
    ...
    var minersTs = cnUtil.get_block_timestamp(shareBuffer, config.poolServer.mergedMiningMinVersion);

    if (hashDiff.ge(blockTemplate.difficulty)){

        apiInterfaces.rpcDaemon('getlastblockheader', {}, function(error, prev){
            
            //!!1
            //trying to identify timewarps
            // 55 sec
            if(prev.block_header){
                timestamp = prev.block_header.timestamp;
                var diffr = minersTs - timestamp;
                if(diffr < 25){
                    log('error', logSystem, 'Timewarp detected at height %d from %s@%s, share type: "%s" - %j', [job.height, miner.login, miner.ip, shareType, error]);
            ...
            apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
                if (error){
                    ...
                    var blockFastHash = cnUtil.get_block_id(shareBuffer, config.poolServer.mergedMiningMinVersion).toString('hex');
                    log('info', logSystem,
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        ...
        case 'getjob':
            ...
            if (junkMiners[miner.login]){
                log('warn', logSystem, 'Miner goes over limit %s@%s', [miner.login, miner.ip]);
                sendReply('Over limit');
                return;
            }
            miner.heartbeat();
